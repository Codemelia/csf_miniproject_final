...
public class UserController {
    ...
    public ResponseEntity<AuthResponse> register(@RequestBody AuthRequest request) {
        ...
        Long id = userSvc.registerUser(request); // convert auth request to user and save to db
    ...
    public ResponseEntity<AuthResponse> login(@RequestBody AuthRequest request) {
        ...
        String token = userSvc.loginUser(request); // converts request to user and validates
    ...
    public ResponseEntity<AuthResponse> register(
        ...
        logger.info(">>> Registering USER: %s".formatted(request.getUsername()));
        try { 
            userSvc.updateUser(request, userId); // convert auth request to user and save to db
            return ResponseEntity.status(HttpStatus.CREATED).body(
                AuthResponse.forRegistration(
